...
int renum(string infile) {
	...
	*/
	...
	ofstream output("probable_configs.pdb");
	while (!input.eof()) {
		getline(input, temp);
		if (!temp.empty()) {
            cout << "got line\n" << endl;
...
static PyObject * _sstmap_probableconfig_run(PyObject * self, PyObject * args)
{
...
static PyObject * _sstmap_renum_probpdb_run(PyObject * self, PyObject * args)
{
    char* standard_cluster_file;
...
static PyMethodDef _sstmap_probableconfig_methods[] = {
    {
        "run_probconfig",
        (PyCFunction)_sstmap_probableconfig_run,
        METH_VARARGS,
        "Run probable congig code"

    },
    {
        "run_renumprobpdb",
        (PyCFunction)_sstmap_renum_probpdb_run,
        METH_VARARGS,
        "Run renumbering of probable config file"
