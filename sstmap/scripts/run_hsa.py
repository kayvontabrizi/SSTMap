...
def parse_args():
    ...
    for index, present in enumerate(files_present):
        if not present:
            sys.exit(
                "%s not found. Please make sure it exits or give the correct path."
                % file_arguments[index]
            )
    ...
    if args.param_file is not None and not os.path.exists(
        args.param_file
    ):  # or not os.path.isdir(args.param_file):
        sys.exit(
            "%s not found. Please make sure it exits or give the correct path."
            % args.param_file
        )
...
def main():
    ...
    h = SiteWaterAnalysis(
        ...
        ligand_file=ligand,
        supporting_file=supp,
        hsa_region_radius=args.hsa_region,
        clustercenter_file=clusters,
        rho_bulk=args.bulk_density,
